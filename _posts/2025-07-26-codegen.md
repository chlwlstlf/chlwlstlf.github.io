...
### <mark class="yellow">보안 강화</mark>

프로덕션 환경에서 GraphQL Introspection을 비활성화할 수 있어 API 구조가 외부에 노출되지 않습니다.

<br>

### <mark class="yellow">브랜치 전환만으로 작업 중인 스키마 접근 가능</mark>

GitHub에서 스키마를 가져오는 방식의 또 다른 장점은 **브랜치만 바꿔서 바로 작업 중인 스키마에 접근할 수 있다**는 것입니다.

기존 Introspection 방식에서는 개발 중인 스키마를 사용하려면 백엔드 개발 서버를 실행해야 했지만 이제는 GitHub URL의 브랜치 부분만 변경하면 됩니다.

```typescript
// 유저 피처 브랜치에 미리 접근하여 스키마 파악 가능
const GITHUB_SCHEMA_URL = `https://raw.githubusercontent.com/org/repo/feat/admin-user/schema.gql`;
```

이를 통해

- **백엔드 서버 실행 불필요**: 서버 없이도 최신 스키마 타입 생성 가능
- **빠른 전환**: 브랜치명만 바꾸면 즉시 해당 환경의 스키마 적용
- **독립적인 프론트엔드 작업**: 백엔드 개발 환경에 의존하지 않고 작업 가능
