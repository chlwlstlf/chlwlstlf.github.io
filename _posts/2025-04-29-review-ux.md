...
직관적이지만 이미지를 여러 개 올릴 수 있는 다른 서비스가 많기 때문에 편의를 위해 여러 장을 업로드하는 것으로 변경하였다.
...
단, 4개로 제한을 두었기 때문에 앞에서부터 4개를 업로드하고 토스트를 띄운다.
...
> concertId 값이 바뀌면 좌석 선택으로 바로 이동한다.
...
> 좌석 선택 dropdown이 자동으로 열리게 한다. seatingId까지 선택되면 특징 선택으로 바로 이동한다.

<br>

1\. dropdown이 하나씩 마운트 되기 때문에 마운트 될 때 자동으로 열리게 한다. 상위 값이 바뀌어 값이 초기화가 되면 autoOpen을 true로 만들어 자동 열림이 뜨게 한다.
...
> features는 여러개 선택할 수 있다. 하나만 선택해도 다음 섹션이 보이지만 스크롤 이동을 하지는 않는다.
...
> images는 사진 여러 개 한 번에 넣을 수 있기 때문에 업로드 하자마자 다음으로 이동한다.
...
> stageDistance 세 개 정보는 라디오 버튼이고 취소가 안 되기 때문에 3개를 다 선택하면 다음으로 이동한다.
...
> obstructions는 features와 동일하게 여러 개 선택할 수 있다. 하나만 선택해도 다음 섹션이 보이지만 스크롤 이동을 하지는 않는다.
...
> contents는 사용자가 textarea에 focusing하면 그 섹션으로 이동한다.
...
페이지가 길기 때문에 작성 완료를 눌렀을 때 유효하지 않은 곳으로 자동 스크롤되면 어떨까? 라는 생각이 들어 바로 `useAutoScroll` 훅을 개선해보았다.
...
유효하지 않은 가장 위에 섹션으로 이동시킨다. 그 섹션이 "작성 완료" 버튼과 가장 멀기 때문에 자동으로 스크롤을 이동하고 이후에 사용자가 직접 아래로 스크롤하며 항목을 채워나가 "작성 완료" 버튼에 가까워지면 UX가 향상될 거라 생각했다.
