...
2\. 동시에 ajax 요청 여러개
  ```jsx
  Promise.all([axios.get('링크1'), axios.get('링크2')])
  .then((결과)=>{  
      console.log(결과.data)
      let copy = [...shoes, ...결과.data];
      setShoes(copy);
...
1\. 문법
  ...
  function TabContent({tab}){ //같은 코드(간결ver.)
...
2\. 문법
  ...
  function TabContent({tab}){ 
    ...
    useEffect(()=>{ 
      setTimeout(()=>{setFade('end')}, 100) //시간 차 두기
      return ()=>{
        setFade('')
      }
    }, [tab])
    return <div className={'start ' + fade}> //2번 className 부착
