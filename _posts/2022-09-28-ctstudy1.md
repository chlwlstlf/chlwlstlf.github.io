...
<div class="blue-box">
  ...
  <p>
    <div>예제 입력 1로 추가 설명</div>
    <div>[1, 3, 6, 6, 7, 9]</div>
    <div>-*-*--**-* → 같은 위치(6)면 생략해도 된다.</div>
    <div>-(*-*)--(**-*) → 집중국1이 거리 2(1~3)를 커버, 집중국2가 거리 3(6~9)을 커버해서 답이 5이다.</div>
  </p>
  <p>
    <div>해결 방법</div>
    <div>센서 덩어리를 효율적으로 나누면 된다.</div>
    <div>효율적이란? 두 센서의 거리가 먼 것부터 덩어리로 나누면 된다.</div>
    <div>덩어리로 나뉜 부분은 집중국이 커버하지 않아도 되기 때문에 첫 번째 센서와 마지막 센서 거리에서 커버하지 않아도 되는 거리를 빼면 된다.</div>
