...
아래는 giscus.html 파일이다. 위에서 만들어준 script 코드와 매우 유사하다. 여기서는 `_config.yml`에 있는 값들을 그대로 받기 때문에 giscus.html 파일을 건드릴 필요는 없고 `"data-repo", "{{ site.repository | downcase }}"` 이런 변수명들을 보면서 여기서 활용되는 `_config.yml`의 변수에 그대로 위의 script 값들을 넣어주면 된다.
...
## <mark class="pink">5. \_config.yml 수정</mark>
